...
// File: contracts/DoodledThings.sol
// Project: DoodledThings
...
contract DoodledThings is  Ownable, ERC721B {
    ...
    // Serum is used to clone DoodledThings and turn them into MutantThings with 3 stages of Mutation.
    // 5 M1 Serums can be combined into 1 M2 Serum but M3 Serums can not be created after the initial snapshot.

    string public provenance;
    string private _contractURI;
    string private _tokenBaseURI;
    address private _signerAddress = 0xA27C5D099f98bBE5087e1572Bd7Ea163816b02dB;
    address private _vaultAddress = 0xA27C5D099f98bBE5087e1572Bd7Ea163816b02dB;
...
// ******************************************************************************************************************************
    ...
    function buy(bytes32 hash, bytes memory signature, uint256 tokenQuantity) external payable {
        ...
        require(tokenQuantity <= DT_PER_MINT, "EXCEED_DT_PER_MINT");
        require(DT_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");

        uint256 supply = _owners.length;
        require(supply + tokenQuantity <= DT_PUBLIC, "EXCEED_MAX_SALE_SUPPLY");
    ...
    function presaleBuy(bytes32 hash, bytes memory signature, uint256 tokenQuantity) external payable {
        ...
        require(matchAddressSigner(hash, signature), "DIRECT_MINT_DISALLOWED");
        require(presalerListPurchases[msg.sender] + tokenQuantity <= DT_PER_MINT, "EXCEED_ALLOC");
        require(tokenQuantity <= DT_PER_MINT, "EXCEED_DT_PER_MINT");
        require(DT_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
...
// ******************************************************************************************************************************
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        ...
        require(supply + receivers.length <= DT_MAX, "MAX_MINT");
        require(giftedAmount + receivers.length <= DT_GIFT, "NO_GIFTS");
